""" Module with class to handle process of simulation"""
...
class Simulation:
    ...
    def __init__(self, environment, rate_infection=.002, rate_decease=.005,
        ...
        self.logger = self.start_logger(GuiSimulator)
        self.hospitalization_chance = hospitalization_chance

    @staticmethod
    def start_logger(GuiSimulator: object = None):
        """ Start logger on file """
        now = datetime.datetime.now().strftime("%Y-%m-%d_%Hh%Mm%Ss")
        header = [
            "day", "total_healthy", "total_infected", "total_incubating",
            "total_deceased", "total_healed", "total_hospitalized",
            "infected_today", "deceased_today", "recovered_today"
        ]
        log_addr = "." + os.path.sep + \
                   "logs" + os.path.sep + "log_" + now + ".csv"
        GuiSimulator.set_log_addr(log_addr)
        return Logger(log_addr, header)

    def get_status(self):
        """ Get summary of status of population """
        return self.environment.get_status()

    def step_time(self, print_status=False, dpg=None):
        """ Execute routines to interaction among agents """
        ...
        recovered, deceased = self.environment.execute_end_of_day(
            self.min_incubation_time, self.min_recovery_time,
            self.rate_decease, self.hospitalization_chance)
        curr_status = self.get_status()

        self.logger.write_to_log_file(
            [self.time_position, curr_status["healthy"],
                curr_status["infected"], curr_status["incubating"],
                curr_status["deceased"], curr_status["healed/immune"],
                curr_status["hospitalized"],
        ...
        dpg.set_value('status_log', 'Processing.. (Starting infection) - Day: '
                        + str(self.time_position)
                        + ' - Healthy(Not contaminated): '
                        + str(curr_status["healthy"])
                        + ' - Infected: ' + str(infected)
                        + ' - Recovered: ' + str(recovered)
                        + ' - Deceased: ' + str(deceased)
                        + ' - Hospit.: ' + str(curr_status["hospitalized"])
                        + ' - Healed/Immune: '
                        + str(curr_status["healed/immune"]))
